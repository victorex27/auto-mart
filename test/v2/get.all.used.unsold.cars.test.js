...
describe('GET /api/v2/car?status=available&state=used', () => {
  ...
  describe('When a user tries to retrieve all used cars without putting the value of state', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&state').set('Authorization', token)
        .send()
        .end((err, res) => {
  ...
  describe('When a user tries to retrieve all used cars with wrong value for state', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&state=uyjkh').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve all used cars ommiting status=available', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?state=used').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve all used cars with proper detail', () => {
    it('should return an object with the status and data', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&state=used').set('Authorization', token)
        .send()
        .end((err, res) => {
