...
use(chaiHttp);
describe('GET /api/v2/car?body_type=coupe', () => {
  ...
  describe('When a user tries to retrieve all cars by body type ommiting body_type value', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?body_type').set('Authorization', token)
        .send()
        .end((err, res) => {
  ...
  describe('When a user tries to retrieve all cars  by body type  with a numeric value', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?body_type=14').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve all  cars  by body type with other parameters that are not allowed', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&body_type=coupe').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve all cars  by body type  with other parameters that are not allowed', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?body_type=coupe&status=available').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
          expect(res.body).to.have.property('error').to.be.a('string').equals('Incorrect number of parameters');
  ...
  describe('When a user tries to retrieve all cars by body type  with proper detail', () => {
    it('should return an object with the status and data', (done) => {
      chai.request(server)
        .get('/api/v2/car?body_type=coupe').set('Authorization', token)
        .send()
        .end((err, res) => {
