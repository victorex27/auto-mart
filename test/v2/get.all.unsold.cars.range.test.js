...
use(chaiHttp);
describe('GET /api/v2/car?status=available&min_price=xxxx&max_price=xxxx', () => {
  ...
  describe('When a user tries to retrieve all cars but misplaced min and max value', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&min_price=50000&max_price=1000').set('Authorization', token)
        .send()
        .end((err, res) => {
  ...
  describe('When a user tries to retrieve all cars but min and max value are the same', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&min_price=1000&max_price=1000').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve all cars but one of the price range is not numeric', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&min_price=amaobi&max_price=1000').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve all cars but one of the price range is negative', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&min_price=-12&max_price=1000').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
          expect(res.body).to.have.property('error').to.be.a('string').equals('Price range should be a positive number');
  ...
  describe('When a user tries to retrieve all cars but one of the price range values is missing', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&max_price=1000').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve a car id that has been sold', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=8&min_price=500&max_price=100000').set('Authorization', token)
        .send()
        .end((err, res) => {
          expect(res.body).to.have.property('status').to.equals(400);
  ...
  describe('When a user tries to retrieve a car with a proper carId that exists', () => {
    it('should return an object with the status and data', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&min_price=500&max_price=100000').set('Authorization', token)
        .send()
        .end((err, res) => {
  ...
  describe('When a user tries to retrieve a car with a proper carId that exists', () => {
    it('should return an object with the status and data', (done) => {
      chai.request(server)
        .get('/api/v2/car?status=available&min_price=1&max_price=4').set('Authorization', token)
        .send()
        .end((err, res) => {
          if (res.body.data.length !== 0) {
            res.body.data.forEach((element) => {
