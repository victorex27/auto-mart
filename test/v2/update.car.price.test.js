...
use(chaiHttp);
describe('PACTH /api/v2/car/:carId/:price', () => {
  ...
  describe('When a user tries to update a car he/she did not post', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .patch('/api/v2/car/2/1000').set('Authorization', token)
        .send()
        .end((err, res) => {
  ...
  describe('When a user tries to update a car with an id that does not exists', () => {
    it('should return an object with the status and error', (done) => {
      chai.request(server)
        .patch('/api/v2/car/90/10000').set('Authorization', token)
        .send()
        .end((err, res) => {
  ...
  describe('When a user tries to update a car advert with valid detail', () => {
    it('should return an object with the status and data', (done) => {
      chai.request(server)
        .patch('/api/v2/car/14/1000').set('Authorization', token)
        .send()
        .end((err, res) => {
